...
export const builder = (yargs: Argv) => {
  return yargs
    .option("x", {
      alias: "priv",
      describe:
        "The private key of user address, required only when creating a new transaction",
      type: "string",
    })
    .option("p", {
      alias: "provider",
    ...
    .option("a", {
      alias: "amount",
      describe:
        "The amount of payment, required only when creating a new transaction",
      type: "string",
    })
    .option("t", {
      alias: "tx",
      describe:
        "The transaction hash. If provided, will use existing transaction hash to add the payment, no other options are required",
      type: "string",
    });
};
